module FoodPornController
  ...
  def do(message_event)
    ...
    http = HTTP.get("https://www.googleapis.com/customsearch/v1", params: { key: Settings.secret.google_search_api.key, cx: Settings.secret.google_search_api.cx, q: word, num: 1, start: rand(1..10), searchType: "image" })
    if http.status != 200
