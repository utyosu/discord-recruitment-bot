module RecruitmentController
  ...
  def open(message_event)
    ...
    Api::Participant.join(recruitment, name: message_event.author.nick, discord_id: message_event.author.id)
    if recruitment['reserve_at'].present?
      send_message_all(message_event, "#{message_event.author.nick}さんから [#{recruitment['label_id']}] を予定時間 #{view_datetime(recruitment['reserve_at'])} で受け付けました。")
    else
      send_message_all(message_event, "#{message_event.author.nick}さんから [#{recruitment['label_id']}] を期限 #{view_datetime(recruitment['created_at'].in_time_zone + EXPIRE_TIME)} で受け付けました。")
    end
  ...
  def close(message_event)
    ...
    JSON.parse(Api::Recruitment.index.body).each do |recruitment, recruitment_index|
      if number == recruitment['label_id']
        Api::Recruitment.destroy(recruitment)
        send_message_all(message_event, "#{message_event.author.nick}さんが [#{recruitment['label_id']}] を終了しました。")
        send_message_all(message_event, recruitments_message)
        TwitterController.recruitment_close(recruitment)
        return
      end
  ...
  def join(message_event)
    ...
    recruitments.each do |recruitment|
      if number == recruitment['label_id'] && !recruitment['participants'].any?{|p|p['discord_id'] == message_event.author.id.to_s}
        Api::Participant.join(recruitment, name: message_event.author.nick, discord_id: message_event.author.id)
        send_message_all(message_event, "#{message_event.author.nick}さんが [#{recruitment['label_id']}] に参加しました。")
        send_message_all(message_event, recruitments_message)
    ...
    JSON.parse(Api::Recruitment.index.body).each do |recruitment|
      ...
      recruitment['participants'].each do |participant|
        if participant['discord_id'] == my_discord_id
          ...
          send_message_all(message_event, "#{message_event.author.nick}さんが [#{recruitment['label_id']}] の参加をキャンセルしました。")
          send_message_all(message_event, recruitments_message)
